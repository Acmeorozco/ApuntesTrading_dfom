...
\subsection{S.A.B - Sucesi\'on de Altos y Bajos}

La \textbf{tendencia} debe ser algo plausible, es decir, indiscutible. Es un criterio por el cual se puede monitorizar el precio. Una tendencia, entonces, es una sucesi\'on de altos y bajos.

\begin{figure}[htb]
    \centering
    \includegraphics[width=11cm]{images/SAB1.png}
    \caption{Sucesi\'on de Altos y Bajos}
    \label{fig:SAB}
\end{figure}

Obs\'ervese, en detalle, lo anteriormente planteado en la figura \ref{fig:SAB}

\begin{figure}[htb]
    \centering
    \includegraphics[width=11cm]{images/SAB2.png}
    \caption{Sucesi\'on de Altos y Bajos}
    \label{fig:SAB2}
\end{figure}

Se ha demarcado, en un color distinto, la traza de la sucesi\'on de altos y bajos para una mejor visualizaci\'on por parte del lector.

Esta sucesi\'on de altos y bajos es la que define la tendencia. Si se rompe esta sucesi\'on, se puede decir que se ha producido un cambio de grado. 

La \textbf{tendencia} puede definirse como el recorrido en el eje de ordenadas (precio) en funci\'on del eje de abscisas (tiempo), por lo tanto, la relaci\'on entre tiempo y recorrido (precio) es la tendencia identificada. Esto es lo que <<manda>>; es lo primero que se identifica; por lo tanto siempre se indica que se debe operar a favor de ella.
